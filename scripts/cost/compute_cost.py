...
def extract_benchmark_summary(results: dict[str, Any]) -> dict[str, Any]:
    """Extract summary metrics from benchmark results."""
    
    # Format 1: Direct summary (from stream_bench.py Phase 2)
    if "requests" in results and "success" in results:
        ...
        # Get total tokens (Phase 2 output includes this directly)
        ...
        # Duration: use duration_seconds if available, else estimate
        ...
        # Include Phase 2 metrics if available
        if "tpot_p50_ms" in results:
            summary["tpot_p50_ms"] = results["tpot_p50_ms"]
        if "tpot_p95_ms" in results:
            summary["tpot_p95_ms"] = results["tpot_p95_ms"]
        if "ttft_p50_ms" in results:
            summary["ttft_p50_ms"] = results["ttft_p50_ms"]
        if "ttft_p95_ms" in results:
            summary["ttft_p95_ms"] = results["ttft_p95_ms"]
        if "avg_output_tokens" in results:
            summary["avg_output_tokens"] = results["avg_output_tokens"]
